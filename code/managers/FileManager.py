...
class FileManager:
    ...
    @staticmethod
    def upload_file_to_s3(file, file_name, bucket_name=default_s3_bucket):
        """
    ...
    @staticmethod
    def renew_file_from_s3(file, old_file_name: UUID, new_file_name: str, bucket_name: str = default_s3_bucket):
        """Delete an old file and uploads a new one"""
        deleted, uploaded, message = False, False, False

        try:
            deleted, message = FileManager.remove_file_from_s3(old_file_name, bucket_name)
            if deleted:
                uploaded, message = FileManager.upload_file_to_s3(file, new_file_name, bucket_name)
